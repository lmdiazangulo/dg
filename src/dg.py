...
class Line:
    ...
    @staticmethod
    def set_nodes_1d(n_order, vertices):
        """ 
...
#            return PL.transpose()
    ...
    def vandermonde_1d(n_order, r):
        ...
        """
        vander_1d = np.zeros([len(r), n_order+1])
        
        for j in range(n_order+1):
            vander_1d[:,j] = Line.jacobi_polynomial(r, 0, 0, j)
    ...
    def jacobi_polynomial_grad(r, alpha, beta, n_order):
        ...
        jacobi_pol = Line.jacobi_polynomial(r,alpha+1,beta+1,n_order-1)
    ...
    @staticmethod
    def vandermonde_1d_grad(n_order,r):
        """
        ...
        grad_vander_1d =  np.zeros([len(r),n_order+1])
        for i in range(n_order+1):
            grad_vander_1d[:,i] = Line.jacobi_polynomial_grad(r,0,0,i)
            
        return grad_vander_1d

    @staticmethod
    def differentiation_matrix_1d(n_order,r,vander_1d):
        """
        ...
        True
        """
        v_r = Line.vandermonde_1d_grad(n_order,r)
        v_inv = np.linalg.inv(vander_1d)
        diff_matrix = np.matmul(v_r,v_inv)
        return diff_matrix

    @staticmethod
    def surface_integral_dg(n_order,vander_1d):
        """
        ...
        v_trans = np.transpose(vander_1d)
        v_i = np.matmul(v_trans,emat)
        lift = np.matmul(vander_1d,v_i)
        return lift

    @staticmethod
    def normals_1d(k_elem):
        """
    ...
    @staticmethod
    def filter_1d(n_order,n_c,s,vander_1d):
        """
        ...
        f_i = np.matmul(vander_1d,np.diag(f_diagonal))
        v_inv = np.linalg.inv(vander_1d)
        filter = np.matmul(f_i,v_inv)
        return filter

    @staticmethod
    def mesh_generator_1d(xmin,xmax,k_elem):
        """
    ...
    def nodes_coordinates(n_order,etov,vx):
        ...
        jgl = Line.jacobi_gauss_lobatto(0,0,n_order)
    ...
    @staticmethod
    def build_maps_1d(n_order,nodes_coord,etoe,etof):
        """
